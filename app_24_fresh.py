...
def get_nodes():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT id, name, port_prefix, created_at, auto_restart, downtime_minutes FROM nodes')
    nodes = []
    for row in c.fetchall():
        ...
        downtime_minutes = row[5] if len(row) > 5 and row[5] is not None else 0
        ...
        try:
            out = subprocess.check_output(['docker', 'exec', container_name, 'curl', '-s', '--max-time', '3', 'http://localhost:11626/info'], text=True, timeout=5)
            ...
            detail = ""
            sync_status = "Meşgul (Yanıt Yok)"

        try:
            dt = datetime.fromisoformat(created_at_str)
            installation_date = dt.strftime("%Y-%m-%d")
            
            total_minutes = (datetime.now() - dt).total_seconds() / 60.0
            if total_minutes < 1: total_minutes = 1
            avail = max(0.0, ((total_minutes - downtime_minutes) / total_minutes) * 100.0)
            availability = f"%{avail:.2f}"
        except:
            installation_date = "Bilinmiyor"
...
def watchdog_loop():
    while True:
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute('SELECT id, port_prefix, auto_restart FROM nodes')
            ...
            for row in monitored_nodes:
                node_id = row[0]
                port_prefix = row[1]
                auto_restart = row[2] if len(row) > 2 else 0
                container_name = f'pi-node-{node_id}'
                node_path = os.path.join(NODES_DIR, node_id)
                ...
                if not is_up:
                    try:
                        ...
                        c_upd.execute('UPDATE nodes SET downtime_minutes = downtime_minutes + 1 WHERE id = ?', (node_id,))
                        conn_upd.commit()
                        conn_upd.close()
                    except:
                        pass
